...
//         useEffect(()=>{
//             fetch("https://dummyjson.com/products/search?q=phone").then((res)=>res.json() ).then((data)=> setProducts(data)).catch((error)=>console.log("cccccccc",error))
//         },[])
...
if (data) {
        ...
        <Container sx={{py:9}}>
            ...
            <Stack direction={"row"} alignItems={"center"} justifyContent={"space-between"} flexWrap={"wrap"}>
                <Box className="mmmmm">
                    <Typography>{data.data.title[0]}</Typography>
                    <Typography variant="h6">
                        Selected 
                        prodects
                    </Typography>
